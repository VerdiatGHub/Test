...
func (f *OnCallProviderFactory) CreateProvider() (core.OnCallProvider, error) {
	...
	} else if f.cfg.OnCall.Provider == "webhook" {
		if f.cfg.OnCall.Webhook.URL == "" {
			return nil, fmt.Errorf("missing URL configuration for webhook on-call provider")
		}

		return NewWebhookOnCallProvider(f.cfg.OnCall.Webhook.URL), nil
	}
